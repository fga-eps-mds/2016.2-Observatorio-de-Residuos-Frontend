describe('SocialServices', function(){
  ...
  beforeEach(inject(function($injector, _googleExtractor_,_facebookExtractor_,_userDataExtractorService_, _$httpBackend_,
  ...
  describe('socialLoginService', function(){
          ...
          it('when existing user login, redirects to home page', function(){
                spyOn($state, 'go');
          ...
          it('should not load the user Data if failed to load pevs from user.', function(){
                spyOn($state, 'go');
                spyOn(currentUserService, 'setUserData');
                spyOn(currentUserService, 'setUserMarking');
                spyOn(currentUserService, 'setUserPevs');
                ...
                $httpBackend.flush();
                expect(currentUserService.setUserData).toHaveBeenCalled();
                expect(currentUserService.setUserMarking).not.toHaveBeenCalled();
                expect(currentUserService.setUserPevs).not.toHaveBeenCalled();
  ...
  describe('userDataExtractorService', function(){
          ...
          it('Should not call facebookExtractor if get an error on get request for user data', function(){
                var fields = 'first_name, last_name, gender, email';
                var userData = {email:'amoedo@gmail.com', first_name:"Amoedo", last_name:'Mito',gender:'male'};
                $httpBackend.expectGET('https://graph.facebook.com/me?fields=' + fields + '&access_token=' + result.credential.accessToken).respond(401, userData);
                spyOn(serviceFacebook, 'extract');
                userExtractorService.extract(result,'facebook');

                $httpBackend.flush();
                expect(serviceFacebook.extract).not.toHaveBeenCalledWith(userData);
          });
          ...
          it('should not call google extractor if get an error on get request for user data', function(){
                var fields = 'first_name, last_name, gender, email';
                var userData = {email:'amoedo@gmail.com', given_name:"Amoedo",family_name:'Mito',gender:'male'};
                $httpBackend.expectGET("https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token="+result.credential.accessToken).respond(401,userData);
                spyOn(serviceGoogle,'extract');
                userExtractorService.extract(result,'google');

                $httpBackend.flush();
                expect(serviceGoogle.extract).not.toHaveBeenCalledWith(userData);
          });
        });
});
